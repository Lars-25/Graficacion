...
def reconocer_letra(hand_landmarks, frame):
    ...
    for i, (x, y) in enumerate(dedos):
        ...
        cv2.putText(
            frame, str(i), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1
        )

    # Dibujar coordenadas del pulgar
    cv2.putText(
        frame,
        f"({int(pulgar[0])}, {int(pulgar[1])})",
    ...
    cv2.line(
        frame,
        (int(pulgar[0]), int(pulgar[1])),
        (int(indice[0]), int(indice[1])),
        (244, 34, 12),
        2,
    )
    ...
    if distancia_pulgar_indice > 50 and distancia_indice_medio > 50:
